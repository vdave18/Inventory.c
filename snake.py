...
directions = {
                curses.KEY_UP: point(-1, 0), curses.KEY_DOWN: point(1, 0), curses.KEY_RIGHT: point(0, 1), curses.KEY_LEFT: point(0, -1)
             }
colors = (
            curses.COLOR_BLUE, curses.COLOR_CYAN, curses.COLOR_GREEN, curses.COLOR_MAGENTA, curses.COLOR_RED, curses.COLOR_YELLOW
        )
...
def start_game():
    ...
    gameover_popup.addstr(1, 11, "Please Try Again!", curses.color_pair(curses.COLOR_YELLOW)) #changed
...
def show_score():
    ...
    score_board.addstr(0, terminal_y//2-5, f"HI: {hi_score:06d}", curses.color_pair(curses.COLOR_RED))
    score_board.refresh()
...
def move_snake():
    ...
    while growth:
        ...
        if new_location in food:
            ...
            growth[0] = color
            growth [:0] = [color] * food[new_location].value*10  #Growth of the snake is also 10 times 

            score += food[new_location].value*10 #Points x10
...
try:
    ...
    game_area.bkgd(curses.A_BOLD | curses.color_pair(curses.COLOR_YELLOW)) #changed
    ...
    print("At least 50x20 character dimensions are required")
    print(f"Current terminal dimension are:{terminal_x}x{terminal_y}")
